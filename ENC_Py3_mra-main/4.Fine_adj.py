...
from seed_rules2 import seed_rules3
...
# Module for calculation of Kappa.
from kappa import kappa
# Module for calculation of Kappa sim.
...
fuzzy_coefficients = base_path + "EU_data\\coeff15.txt"
# Specify the fuzzy transition weights for the calculation of FKS.
fuzzy_trans_coefficients = base_path + "EU_data\\coefficients15.txt"
...
smap_path = working_directory + "Log\\Land_use\\Land use map_2016-Jan-01 00_00_00.rst"
# Set the land-use class names.
luc_names = ["Agriculture", "Forest", "Industry",
             "Nature", "Recreation", "Urban-formal",
             "Urban-informal", "Urban-villages", "Transportation", "Airport and Harbour", "Water","Outside"]

# Set the socio-economic group for each land-use class. This is used to set the
# order of calibration.

socio_eco_group = ["Other", "Other", "Work", "Other", "Recreation",
                   "Residential", "Residential", "Residential", "Other",
                   "Other", "Other","Other"]
...
points = np.loadtxt(point_levels_file_path)
tails = np.loadtxt(tail_levels_file_path)
...
if seed is True:
    ...
    theta_cp = 0.0275
    theta_ct = 0.02
    rules = seed_rules3(luc_names, luc, act, pas)
...
while run_count < max_runs:
    ...
    # Calculate the AAWCE.
    clu_temp[run_count] = area_weighted_clu_error(amap, smap, mask, luc, pas,act, luc_count)
    # Calculate the run Kappa.
    kappa_temp[run_count] = kappa(amap, smap,mask)
    # Calculate the run Kappa Sim.
...
output_duration_file = (output_path + "\\Fine_cal_output\\" 
                        "duration.txt")
...
with open(log_output_file, "w", newline='') as csv_file:
    ...
    values = ["From", "To", "Distance", "Value", "Kappa", "KSIM", "AAWCE", "WS"]
    writer.writerow(values)
    # Now write the log
